...
def main(
    ...
    gs_optimization_param_path: str = data_cf["gs_optimization_param_path"]
    ...
    console.print(f"[yellow] img_rows: {img_rows} [px]")
    console.print(f"[yellow] img_cols: {img_cols} [px]")
    console.print(f"[yellow] depth_scaling: {depth_scaling}")
    console.print(f"[yellow] num_frames: {len(reader)}")
    ...
    geo_wrapper.GSSavePointCloud(f"{results_dir}/gs_{timestamp}")
