...
class MotionFilter:
    ...
    @torch.amp.autocast('cuda',enabled=True)
    @torch.no_grad()
    def track(self, tstamp, image, intrinsics=None, depth=None):
        """ main update operation - run on every frame in video
        
        Args:
            tstamp: frame timestamp/index
            image: [1, 3, H, W] color image
            intrinsics: [4] camera intrinsics (fx, fy, cx, cy)
            depth: [H, W] depth map from dataset (meters). Used as mono depth prior.
        """
        ...
        if self.video.counter.value == 0:
            ...
            # Use dataset depth directly as the depth prior
            mono_depth = depth.to(self.device) if depth is not None else None
